...
known facts causally flip the base model at layer~0 (median), latent at layer~10,
unknown at layer~16, and synthetic at layer~19 out of 24.
Furthermore, logit-lens trajectories confirm a clear structural boundary: LoRA shifts 
predictions \emph{earlier} for conditions with latent signal (known $-2.5$ layers) 
and \emph{later} for novel facts (synthetic $+3.8$ layers).
All gains come at a steep locality cost: Kullback-Leibler (KL) divergence on 
neighboring-fact predictions exceeds 6.4 across all conditions, proving LoRA 
induces massive representational drift regardless of the knowledge type.
\end{abstract}
...
\begin{itemize}
  ...
  \item \textbf{Latent (rank 2--5, log-prob $> -5$):} The correct answer
    ...
    $p \gtrsim 0.007$; 2{,}837 facts; 16.1\%).
...
masked), AdamW with learning rate $10^{-4}$, batch size 16, for 5 epochs (535
optimizer steps).
...
The paraphrase gain ($\Delta = +0.613$) is the \emph{largest of any condition},
exceeding even known facts ($\Delta = +0.497$).
...
The log-probability gate (excluding 35 rank-2--5 facts with near-zero
probability) sharpens the latent category: only facts with genuine sub-threshold
...
\midrule
Known     & 100/100 &  0.13 &  0 \\
Latent    & 100/100 &  9.89 & 10 \\
Unknown   &  82/100 & 15.29 & 16 \\
Synthetic &  64/100 & 18.28 & 19 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:patching} and Figure~\ref{fig:patching} provide the causal confirmation. By patching the full residual stream, we track when the target token accumulates enough LoRA-modified signal to independently determine the output. The predicted ordering is exactly realised: known (median 0) $<$ latent (10) $<$ unknown (16) $<$ synthetic (19).

For \textbf{known} facts, all 100 flip at layer 0 --- the answer is already
causally determined before any transformer block runs.
For \textbf{latent} facts, 100/100 flip at a median of layer~10, placing the
causal site in the mid-network region. LoRA's intervention here is minimal: 
it redirects an existing activation rather than writing a new one.
For \textbf{unknown} facts, 82/100 flip at a median of layer~16; for \textbf{synthetic}
facts, 64/100 flip at a median of layer~19.
...
($r=4$: 22.2, $r=32$: 22.1).
For \textbf{unknown} facts the pattern is similar (47.2\% to 69.2\%; depth
stays near 21).
...
and paraphrase accuracy plateaus around $r=8$ (82.0\%).
...
\begin{table}[t]
\centering\small
\caption{Neighborhood representational drift across 5{,}000 related-fact
prompts per condition.
\emph{KL div.}\ = mean KL$(P_\text{base} \| P_\text{LoRA})$ over the full
output vocabulary (lower = less disruption).
\emph{Pres.\ rate} = fraction of prompts where LoRA top-1 equals base top-1
(reported for reference; KL divergence is the primary metric as it captures
distributional drift beyond top-1 changes).}
\label{tab:locality}
\begin{tabular}{lccc}
\toprule
\textbf{Condition} & \textbf{KL div.} & \textbf{Pres.\ rate} & \textbf{$N$ prompts} \\
\midrule
Known          & 6.42 & 0.223 & 5{,}000 \\
Latent (top-5) & 6.47 & 0.102 & 5{,}000 \\
Unknown        & 6.56 & 0.063 & 5{,}000 \\
\bottomrule
\end{tabular}
\end{table}
...
We report KL$(P_\text{base} \| P_\text{LoRA})$ over the full vocabulary as
the primary metric: unlike top-1 preservation, it captures distributional drift
even when the top-1 token does not change (e.g.\ when LoRA shifts two
near-equal candidates from 51/49 to 49/51).
Known facts show a mean KL of 6.42 nats, still severely disrupted but the
lowest of the three conditions.
Latent (6.47) and unknown (6.56) are close to each other and substantially
higher.
The top-1 preservation rates tell the same story more starkly: 22.3\% for
known, 10.2\% for latent, and 6.3\% for unknown.
The latent condition is particularly instructive: despite encoding at a
shallower layer (median 10) than unknown (median 16), its KL divergence is
nearly as high.
...
Unknown-fact fine-tuning, operating in mid-to-deep layers, causes maximal
...
($\Delta = +0.613$, exceeding even known facts at $+0.497$), yet their
...
prediction.
The log-probability gate used to define the latent category (answer log-prob
$> -5$) reinforces this interpretation: the 35 reclassified facts that passed
the rank-2--5 criterion but carried near-zero probability are semantically
closer to unknown, and excluding them sharpens the latent cluster.
...
Crucially, all of these interventions carry severe side effects. The KL-divergence baseline ($\sim 6.5$) exposes that LoRA's modifications ripple aggressively through the model's surrounding associative networks, rewriting the probability spaces of neighboring facts. While known facts preserve slightly more top-1 predictions than unknown facts, the absolute magnitude of the KL divergence mathematically proves that LoRA is not a surgical intervention; it severely degrades the model's representation of related concepts.
...
and 74.5\% paraphrase generalisation --- higher than any other condition ---
from the smallest structural change.
All conditions degrade locality severely, with mean KL-divergence between base
and LoRA output distributions ranging from 6.42 nats (known) to 6.56 nats
(unknown), and top-1 preservation rates of 22.3\% down to 6.3\%.
